...
Run:  python examples/cameras/smoke_all.py --prim /World/rsd455/RSD455/Camera_OmniVision_OV9782_Color
...
def main():
    ...
    # -- pose (round-trip: read, write same back) ------------------------------
    ...
    if wp and wp.get("position") is not None:
        _record(
            ...
            *_call(
                ...
                "PUT",
                f"{root}/world_pose",
                {"position": wp["position"], "orientation": wp["orientation"], "camera_axes": "world"},
            ),
        )
    else:
    ...
    _roundtrip(base, f"{root}/horizontal_aperture", f"{root}/horizontal_aperture", "horizontal_aperture")
    ...
    if clip and clip.get("clipping_range"):
        ...
        _record(
            ...
            *_call(base, "PUT", f"{root}/clipping_range", {"near_distance": near, "far_distance": far}),
        )

    # -- string-valued config (round-trip; skip if unset) ----------------------
    ...
    _roundtrip(base, f"{root}/lens_distortion_model", f"{root}/lens_distortion_model", "lens_distortion_model")
