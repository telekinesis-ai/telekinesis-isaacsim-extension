...
Flow (all over HTTP):
  1. PUT /prims/poses/default (body: prim_path) -> record the current pose as default
  2. POST /prims/poses/relative -> nudge the prim away from it
  3. POST /prims/poses/default/reset (body: prim_path) -> snap back to the default
...
def main():
    ...
    _request(base, "POST", "/prims/poses/default/reset", body=args.prim)
