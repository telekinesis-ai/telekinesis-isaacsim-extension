...
from .comm.connection_server import ConnectionServer
from .global_variables import EXTENSION_DESCRIPTION, EXTENSION_TITLE
from .ui.ui_builder import UIBuilder

# Well-known entry point. Clients connect here, name a prim (+ optional URDF), and
# get back a dedicated per-device server port. Multi-robot scales by adding more
# connect calls -- the connection server tracks one device server per prim.
...
class Extension(omni.ext.IExt):
    def on_startup(self, ext_id: str):
        ...
        # Host the single connection server. Clients connect here and get a
        # dedicated per-device server port back (see ConnectionServer). A bind
        # failure (e.g. port already in use) must never break startup.
        self._connection_server = ConnectionServer(port=CONNECTION_PORT)
        try:
            self._connection_server.start()
        except Exception as exc:
            print(f"[bridge] connection server on port {CONNECTION_PORT} failed to start: {exc}")
