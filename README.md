...
**Installed Isaac Sim, want to just use it:** open **Window ▸ Extensions**, search for
`telekinesis.isaacsim.bridge`, and enable it. Isaac Sim will install `fastapi`, `uvicorn`,
`pydantic`, and `websockets` automatically on first load via `pipapi`.

**Building from this source tree instead (contributing, or testing an unreleased
change):** follow [DEVELOPMENT.md](DEVELOPMENT.md) to point Isaac Sim at this repo's
`exts/` folder directly.
...
The full, always-current endpoint list is generated straight from the code (nothing here is
hand-maintained, so it can't drift the way a hand-written table can):

- **Bridge running:** open `http://127.0.0.1:8766/docs` (Swagger UI) or `/redoc` -- live,
  interactive, generated by FastAPI from the actual routes.
- **Browsing without Isaac Sim running:** [API reference](https://telekinesis-ai.github.io/telekinesis-isaacsim-extension/) --
  a static copy of the same schema, regenerated on every release (see
...
(see [DEVELOPMENT.md](DEVELOPMENT.md#error-handling) for the status-code convention).
